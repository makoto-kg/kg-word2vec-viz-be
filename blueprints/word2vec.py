...
@word2vec_bp.route("/words", methods=["GET"])
def words():
    if request.args.get("q") is not None:
        filtered = list(
            filter(lambda word: request.args.get("q") in word, word_list)
        )
...
@word2vec_bp.route("/vec", methods=["GET"])
def word2vec():
...
@word2vec_bp.route("/similar", methods=["GET"])
def similar():
    # pos, neg
